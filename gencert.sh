...
if [ ! -f "$DIR/$CA.key" ]; then
    ...
    openssl x509 -sha1 -noout -in keys/$CA.pem -fingerprint | sed 's/SHA1 Fingerprint=//g' > keys/$CA.fp
...
openssl x509 -sha1 -noout -in keys/$NAME.pem -fingerprint | sed 's/SHA1 Fingerprint=//g' > keys/$NAME.fp
